...
from .models import QueuedDesktop, QueuedMobile, Order
from .forms import *
...
def add_items(request, pk, cust):
    context = {
        'pk': pk,
        'cust': cust,
        'tub_list': QueuedDesktop.objects.filter(order_id=pk),
        'bottle_list': QueuedMobile.objects.filter(order_id=pk),
    }
    return render(request, 'sales/add_items.html', context)


class NewQueuedDesktop(CreateView):
    model = QueuedDesktop
    form_class = NewQueuedDesktopForm

    def get_success_url(self):
        return reverse('sales:add_items', args=(self.kwargs['pk'],))

    def get_context_data(self, **kwargs):
        context = super(NewQueuedDesktop, self).get_context_data(**kwargs)
...
def get_tubs(request, pk):
    tub_set = QueuedDesktop.objects.filter(order_id=pk)
    return render(request, 'sales/get_tubs.html', {'tub_set': tub_set})


def get_bottles(request, pk):
    bottle_set = QueuedMobile.objects.filter(order_id=pk)
    return render(request, 'sales/get_bottles.html', {'bottle_set': bottle_set})


class NewQueuedMobile(CreateView):
    model = QueuedMobile
    form_class = NewQueuedMobileForm

    def get_context_data(self, **kwargs):
        context = super(NewQueuedMobile, self).get_context_data(**kwargs)
        context['pk'] = self.kwargs['pk']
        return context
...
class OrderDetail(DetailView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context['date'] = d
        tubs = QueuedDesktop.objects.filter(order_id=self.kwargs['pk'])
        used_tubs = []
        context["tubs"] = {}
        for tub in tubs:
        ...
        bottles = QueuedMobile.objects.filter(order_id=self.kwargs['pk'])
        used_bottles = []
        context["bottles"] = {}
        ...
        context['tub_total'] = QueuedDesktop.objects.filter(order_id=self.kwargs['pk']).count() * 400
        context['bottle_total'] = QueuedMobile.objects.filter(order_id=self.kwargs['pk']).count() * 200
